...
while 1:
	...
	#char2 = read(2)
	#char3 = read(3)-offset
	data0 = (mapVoltage(char0,0,1023,0,3.3)*vgen)-fudge #Voltage measurement (Generator)
	data1 = mapVoltage(char1,0,1023,0,3.3)*vgain #Voltage measurement (Solar Panel)
	#data2 = mapVoltage(char2,0,1023,0,3.3)/vpA #Current measurement (Solar Panel)
	#data3 = mapVoltage(char3,0,1023,0,Vcc)/vpA #Current measurement (Generator)
	print(roundValue(data0)," V", " | ", roundValue(data1), " V" )
	#print(time.strftime("%Y-%m-%d %H:%M:%S"),roundValue(data0),',',roundValue(data1),',',roundValue(data2),',',roundValue(data3))
	saveFile(time.strftime("%Y-%m-%d %H:%M:%S"),roundValue(data0), roundValue(data1))
	time.sleep(1)
